...
def discretize(bounds, n_squares):
    ...
    for i in range(len(xs)-1):
        for j in range(len(ys)-1):
            ...
            squares[i, j] = {'vertices': vertices, 'center': np.array(square.centroid),
                             'i': i,'j': j,'seen': False, 'square': square}
    return squares
...
def sample_feasible_point(target_pos, repeater_pos, desired_range, sensor_range, repeater_v_max, boundary_obstacles):
    ...
    for sample in samples:
        ...
        dist_to_closest_obst = np.inf
        line = geometry.LineString([target_pos, sample])
        for square in squares_of_interest:
            dist_to_line = geometry.Point(square['center']).distance(line)

            if dist_to_line < dist_to_closest_obst:
                dist_to_closest_obst = dist_to_line
        sample_dmin.append(dist_to_closest_obst)
...
while not done:
    ...
    if repeaters:
        ...
        for r in range(len(repeaters)):
            ...
            if r == 0:
                ...
                repeaters[r].pos_desired = sample_feasible_point(target_pos, repeater_pos, desired_range,
                                                                 sensor_range, v_max, boundary)
            else:
    ...
    if time_step%plot_step == 0:
        plotter.set_bg(repeaters, squares, pos_main_drone, bounding_lines, obstacles, sensor_range,
                       scanning_range, ground_station, desired_range)
        pg.display.flip()